...
| `Economy.LoanCapitalizationCeiling` | 2.0 | (Raise) a loan may capitalize further past its issued size before being force-defaulted — runaway principals climb higher, debt tails run longer. | (Lower) a loan is written off sooner once its principal outgrows the original — collateral is seized earlier, less unpayable debt compounds toward millions. |
...
| `Economy.MaxDebtToIncomeRatio` | 3.0 | (Raise) borrowers may stack more open-loan principal (up to N× one epoch's receipts) before the credit-score gate locks them out — deeper debt, more default risk. | (Lower) tighter creditworthiness ceiling; already-indebted polities are cut off from new loans sooner, leaning harder on sovereign issuance. Default 3.0 is a first pass — expect sweep-driven adjustment. |
...
| `Economy.HaulingDiscountFloor` | 0.05 | (Raise) the fuel-grounded hauling discount bottoms out higher, so even a good whose freight-fuel cost exceeds its value keeps a larger residual siting score — distant workings stay in contention on raw richness. | (Toward 0) a working whose output value is fully eaten by freight fuel is scored near zero, so only bodies whose richness clears their haul cost site at the frontier; the port hex and near neighbors (near-zero freight) dominate. |
| `Economy.DispersionWeight` | 0.6 | (Raise) a second same-class extraction working (2nd mine, 2nd skimmer, …) is penalized harder for sitting on or beside the builder's first — extraction fans farther off the port body, worked hexes reach deeper into the domain, and more outposts clear the G=2 graduation gate. Bounded: a same-hex working keeps `1 − this` of its score, so it never zeroes a rich isolated site. Same-class only (support/processing keep port-body affinity). | (Toward 0) recovers pre-dispersion port-clustered siting — the 2nd/3rd same-class working stacks beside the first on the port body, domains stay a point, and graduation cannot fire. |
...
| `Relations.FederationCredibilityDiscount` | 0.20 | Strong-backed allies fuse more readily — a debtor partner drags the pair's `min` credibility to ~0. | Monetary standing is diplomatically inert. |
...
| `Relations.VassalAbsorptionCredibilityDiscount` | 0.20 | A monetarily weak vassal under a credible overlord is absorbed at a lower warmth bar. | Overlord/vassal credibility gap has no bearing on absorption. |
...
| `Expansion.GraduationMarginHexes` | 1 | Wider dead gap between a graduating outpost's future port and every existing one — frontier outposts must sit farther out before they can become starports; interior density stays subordinate longer. The gate is the LITERAL anti-adjacency spacing `G = 1 + this` (default 2): eligible iff `dist ≥ G` from every port core. NOT radius-derived — two earlier domain-scale formulations blocked graduation entirely because outposts form only 1–3 hexes from their parent (§4, decision #2 FINAL). | Ports may graduate closer together; at 0 → G=1 an outpost may touch a port and still graduate. |
| `Expansion.GraduationYears` | 5 | The administrative promotion of a frontier outpost into a starport takes longer — its discounted cost streams from `ExpansionPoints` as construction wages over more world-years (P7; no convoy, no fuel, no goods). | Snappier promotions; the port lands sooner and the wage stream is briefer. |
| `Expansion.GraduationCostDiscountPerFacility` | 0.08 | Each existing facility at the hex knocks more off graduation cost — a works-heavy outpost graduates far cheaper (it is "already half a colony"). Floored by `GraduationMinCostFraction`. | Facilities barely discount; even a developed outpost pays near-full price. |
| `Expansion.GraduationCostDiscountPerResident` | 0.05 | Each unit of resident population knocks more off graduation cost — a populous outpost graduates cheaper. Floored by `GraduationMinCostFraction`. | Population barely discounts; a crowded outpost still pays near-full price. |
| `Expansion.GraduationMinCostFraction` | 0.4 | The floor rises — however developed, promotion never costs less than this share of `ColonyCost` (raising a starport is never free). | The floor drops; a fully-developed outpost can graduate almost for free. |
| `Expansion.GraduationScoreBonus` | 0.3 | Infill weighs more heavily against reach in the one ranked expansion list — a frontier outpost outranks a comparable fresh colony target sooner, so polities fill in before they leap out. | Reach dominates; graduation only wins on genuinely richer terrain. |
