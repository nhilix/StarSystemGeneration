...
| `Economy.FxSensitivity` | 1.0 | (Slice CU-1; not yet wired) currency numeraire rates swing harder on supply/output imbalance — a minting spree devalues a currency faster against its peers. | (Toward 0) rates go rigid, approaching a fixed 1:1 across currencies. |
| `Economy.FxReceiptsFloor` | 1.0 | (Slice CU-1; not yet wired) a higher floor damps the FX rate of a near-zero-receipts currency, keeping a freshly-split polity's money from spiking. | (Toward 0) low-output currencies can blow their rate up (and risk divide-by-zero) — the floor is the guard. |
