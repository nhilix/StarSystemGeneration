...
public class EpochEngineTests
{
    ...
    public void Interior_EntersActorsAtTheirEntryEpoch_ChronicleFinalizesTheEvent()
    {
        ...
        // every actor either emerged on the schedule or seceded (slice G)
        var schisms = state.Log.Events
            ...
            .ToHashSet();
        Assert.Equal(state.Actors.Count, emergences.Count + schisms.Count);
        foreach (var a in state.Actors)
        {
            if (schisms.Contains(a.Id)) continue;
