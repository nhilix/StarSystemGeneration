...
public static class MetricsOps
{
    ...
    public static MetricRow Snapshot(SimState state)
    {
        ...
        // conservation (spec §2, widened by monetary-equilibrium design §5):
        // there are now TWO declared mints — the one-time entry endowment and
        // bounded sovereign issuance — so the supply delta minus the epoch's
        // endowments AND its issuance must be zero. Both are diffed as levels
        // against the previous row; every other new flow (pool decay, wealth
        // levy) is a symmetric transfer between existing holders, not a mint.
        // A fresh series (a loaded artifact) has no baseline: its first
        // residual is defined 0.
        int endowed = 0;
